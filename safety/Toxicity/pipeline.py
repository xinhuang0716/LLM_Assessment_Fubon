...
from deepteam.vulnerabilities import Toxicity, IllegalActivity, PersonalSafety
...
def runEvaluation(target_llm: Callable, simulator_model: DeepEvalBaseLLM, evaluation_model: DeepEvalBaseLLM, sample_size: int):
    ...
    risk_assessment = red_team(
        ...
        #vulnerabilities=[vul_0, vul_1, vul_2],
    ...
    file_name_log = f"./results/toxicity_log_{time.strftime("%Y%m%d")}.json"
    file_name_result = f"./results/toxicity_summary_{time.strftime("%Y%m%d")}.json"
